...
function ext = load_nii_ext(filename)
   ...
   if length(filename) > 2 && strcmp(filename(end-2:end), '.gz')
	 ...
	 ~strcmp(filename(end-6:end), '.nii.gz')
      ...
      elseif strcmp(filename(end-6:end), '.nii.gz')
         ...
         filename = char(filename);	% convert from cell to stringg
      end
   end
   ...
   if ~isempty(strfind(filename, '.nii')) && strcmp(filename(end-3:end), '.nii')
      new_ext = 1;
      filename(end-3:end)='';
   end

   if ~isempty(strfind(filename, '.hdr')) && strcmp(filename(end-3:end), '.hdr')
      filename(end-3:end)='';
   end

   if ~isempty(strfind(filename, '.img')) && strcmp(filename(end-3:end), '.img')
      filename(end-3:end)='';
   end
